...
float readUntilStable(bool z, float offset);
void reise();
uint8_t led(float p);
...
void loop() {
  ...
  if (p > zOut/2) {
    pO = readUntilStable(false,zOut);
    if (pO > 0 && !digitalRead(13)) {
      leggi(pO);
    }
  }
  else if (p < zIn/2) {
    pO = readUntilStable(false,zIn);
    if (pO < 0 && !digitalRead(13)) {
      leggi(pO);
    }
  }
